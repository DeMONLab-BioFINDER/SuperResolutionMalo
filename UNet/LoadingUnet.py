...
"save_res_ratio" : 10000, #steps at which to save the output and perceptual metrics, must be a multiple of save_loss_ratio
...
elif settings["scheduler"]=="reducePlateau":
    scheduler = ReduceLROnPlateau(optimizer, patience=5,cooldown = 5,threshold = 5e-4,mode='min')
...
for epoch in range(n_epochs):
    ...
    for step,batch in batches:
        ...
        reconstruction = my_unet(in_img,timesteps = torch.ones(sb,dtype=torch.float32).cuda(),context=context)
        ...
        if not(iii%save_loss_ratio):
            ...
            print("epoch : ",epoch+1," step : ",step," mean25 train l1 loss : ",np.sum(np.array(toutou[-save_loss_ratio:]))/save_loss_ratio," mean25 train perc loss : ",np.sum(np.array(toutou2[-save_loss_ratio:]))/save_loss_ratio)
        ...
        if not(iii%save_res_ratio) or not(iii%tot_step):
            ...
            with torch.no_grad():
                ...
                for i_test in range(n_test):
                    ...
                    #print("corruption shift : ",shifting,bott)
                    ...
                    nib.save(nib.Nifti1Image(res_i_test, af_test[i_test], header=hd_test[i_test]),"results/trial"+str(n_trial)+"/images/"+str(df_name_test["patients_7T_test"][i_test])+"_7T_step_"+str(iii)+".nii.gz")
            ...
            print("time : ", time.time()-toc," nb steps : ",past_epoch*n_slices_train+iii,"/",(n_epochs+past_epoch)*tot_step, "  train L1 loss : ", round((epoch_loss/(step+1)),4), " train perc loss : ", round((epoch_perc_loss/(step+1)),4),"  test L1 loss : ", round(test_l1_loss/(step_t+1),4), " test perc loss : ", round(test_perc_loss/(step_t+1),4))
            my_unet.train()
...
plt.plot(save_loss_ratio*np.array(range(1,len(lrs)+1)),-np.log10(np.array(lrs))) 
plt.title("Learning rate, trial number "+str(n_trial))
plt.xlabel("step")
plt.ylabel("-log(lr)")
