...
from WarvitoCodes.models_attention_2D import SpatialTransformer
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm = Normalize(channels,num_groups)
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        self.out_layers = nn.Sequential(
            Normalize(self.out_channels,num_groups),
            nn.SiLU(),
    ...
    def _forward(self, x, emb):
        ...
        if self.use_ada:
            ...
            h = h*torch.exp(self.phi(torch.log(s)))
...
class UNetModel(nn.Module):
    def __init__(
        ...
        self.use_initial_downsample = use_initial_downsample #If your image is too big for the RAM
        ...
        if self.use_initial_downsample:
            self.input_blocks = nn.ModuleList(
                [
                    TimestepEmbedSequential(
                        Downsample(in_channels, use_conv = True , out_channels = model_channels)
                    )
                ]
            )
        else :
                ...
                if ds in attention_resolutions:
                    ...
                    layers.append(
                        AttentionBlock(
                            ...
                            ch, num_heads, dim_head, depth=transformer_depth,num_groups=num_groups, context_dim=context_dim
                        )
                    )
        ...
        if self.use_initial_downsample:
            self.final_up = Upsample(ch,conv_resample,out_channels = ch)
