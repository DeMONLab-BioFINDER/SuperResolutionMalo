...
n_trial = 0 #create a folder "resultsUnet" with folders "triali" where i=n_trial to save loss curves, "params.txt"  saves the settings###############
...
"n_channels" : 256, #number of channels after initial convolution
"num_groups" : 64, #size of batch in batch normalization
...
"attention_res" : [4,8], #Indicate at which resolution to include a cross attention layer, if empty then no cross attention. Can take a lot of RAM if used in high stages. 

"size_batch" : 56, #reduce for RAM limitations, increase for less noise
"num_workers" : 16, #number of threads used, divides size_batch
"n_in_slices" : 3,  #Number of slices in the input, default is 1.

"lr" : 1e-4, #initial learning rate 
"scheduler" :  "decay", #None, "decay" or "reducePlateau" or None
"decay" : (1,0.5), #learning rate decay
"betas" : (0.9,0.999), #betas of the optimizer, beta1=0.9 nice for sparse gradients
"dropout" : 0, # not good for SR 

"conditions" : ["Sex","Age","position"],
...
"test_size" : 13, 
"train_size"  : 123,
"n_slices" : 290, #########################coronal290 axial244 sagittal224 
"c_dim" : 1, #Dimension of the context
"slice_dim" : 0, #Dimension along which to divide the image in slices##########coronal0 axial1 sagittal2
"slice_size" : (242,222), #Image of the input and output images, before padding##########coronal(242 222) axial(288 222) sagittal(288 242)
...
border_bot = [50,61,26]
bott = border_bot[view_id]################
border_top = [337,302,247]
topp = border_top[view_id]################

n_colors = settings["n_in_slices"]

test_padder = (26,24,60,48,50,14)
#Save the parameters
with open("resultsUnet/trial"+str(n_trial)+"/params.txt", 'w') as f:
    f.write(str(settings))



big_path = "/proj/berzelius-2024-156/users/x_rushe/super-resolution/"
data_dir = big_path + "Unet/contents_ano_" + str(view_id) + "/"
...
info = pd.read_csv(data_dir+"patient_info_anon_0"+".csv")########## 012

# Remove empty unnamed columns caused by trailing commas
info = info.loc[:, ~info.columns.str.match(r"^Unnamed: \d+$")]

# Create the subject index expected by the original code:
# anon_0000 -> 0, anon_0001 -> 1, ...
if "Unnamed: 0" not in info.columns:
    info["Unnamed: 0"] = info["anon_id"].str.extract(r"anon_(\d+)").astype(int)


############
bot_col = "bot_" + str(view_id)
top_col = "top_" + str(view_id)
bot_abs_col = "bot_abs_" + str(view_id)

if bot_abs_col not in info.columns:
    info[bot_col] = pd.to_numeric(info[bot_col])
    info["bot_first_slice"] = pd.to_numeric(info["bot_first_slice"])
    info[bot_abs_col] = np.maximum(info[bot_col], info["bot_first_slice"])
#############


print(info.columns)
#print(info[["anon_id", "Unnamed: 0", "bot_1", "bot_first_slice", "bot_abs", "top_1"]].head())
print(info[["anon_id", "Unnamed: 0", bot_col, "bot_first_slice", bot_abs_col, top_col]].head())
...
for i in range(136):
    if i in idx_test:
        ...
        #bot1 = info.loc[info["Unnamed: 0"]==i,"bot_"+str(1)].iloc[0]
        #bots_test.append(max(bot1,info.loc[info["Unnamed: 0"]==i,"bot_abs"].iloc[0]))
        bot1 = info.loc[info["Unnamed: 0"] == i, bot_col].iloc[0]
        bots_test.append(max(bot1, info.loc[info["Unnamed: 0"] == i, bot_abs_col].iloc[0]))
    else :

        #top1 = info.loc[info["Unnamed: 0"]==i,"top_"+str(1)].iloc[0]
        #bot1 = info.loc[info["Unnamed: 0"]==i,"bot_abs"].iloc[0]

        top1 = info.loc[info["Unnamed: 0"] == i, top_col].iloc[0]
        bot1 = info.loc[info["Unnamed: 0"] == i, bot_abs_col].iloc[0]
...
contexts = np.load(data_dir+"contexts1_s"+str(n_colors)+".npy")
print(contexts.shape,contexts)
contexts = np.concatenate((contexts[:,:2,:],contexts[:,3:4,:]),axis=1)
print(contexts.shape,contexts)
...
#ranges = np.load(big_path+"Unet/contents_ano_1/ranges1_s"+str(n_colors)+".npy")
#ranges_test = ranges[idx_test]
#ranges_train = ranges[idx_train]
ranges_test = np.ones(13)
ranges_train = np.ones(136)
...
for i in range(136):
    if i in idx_test:
        I3T = nib.load(data_dir+"images/hd_af_"+str(i)+"_1_s"+str(n_colors)+".nii.gz")
...
df_name_test.to_csv("resultsUnet/trial"+str(n_trial)+"/test_patients.csv",index=False)


del I3T

x = np.load(data_dir+"inputs1_s"+str(n_colors)+".npy")
...
y = np.load(data_dir+"outputs1_s"+str(n_colors)+".npy")
y[y>1.5] = 1.5
...
for epoch in range(n_epochs):
    ...
    for step,batch in batches:
        ...
        if not(iii%save_loss_ratio):
            ...
            print("epoch : ",epoch+1," step : ",step," mean25 train l1 loss : ",round(moumou[-1],4)," mean25 train perc loss : ",round(moumou2[-1],4),flush=True)
    ...
    toutous.to_csv("resultsUnet/trial"+str(n_trial)+"/moutous.csv",index=False)        
    ...
    if (epoch+1) in saved_epoch:
        ...
        with torch.no_grad():
            ...
            for i_test in range(n_test):
                ...
                df_test.to_csv("resultsUnet/trial"+str(n_trial)+"/test_metrics.csv",index=False)
                    
                res_i_test = torch.transpose(res_i_test,0,1)
                res_i_test = F.pad(res_i_test, test_padder, "constant", 0)
                #nib.save(nib.Nifti1Image(res_i_test, af_test[i_test], header=hd_test[i_test]),"resultsUnet/trial"+str(n_trial)+"/images/"+str(idx_test[i_test])+"_7T_step_"+str(iii)+".nii.gz")
        ...
        df.to_csv("resultsUnet/trial"+str(n_trial)+"/losses.csv",index=False)

        print("time : ", time.time()-toc," nb steps : ",iii,"/",(n_epochs+past_epoch)*tot_step, "  train L1 loss : ", round((epoch_loss/(step+1)),4), " train perc loss : ", round((epoch_perc_loss/(step+1)),4),"  test L1 loss : ", round(test_l1_loss/(step_t+1),4), " test perc loss : ", round(test_perc_loss/(step_t+1),4))
...
with open("resultsUnet/trial"+str(n_trial)+"/training_time.txt", 'w') as f:
    f.write(str(time.time()-tic))

if settings["save_model"]:
    torch.save({"epoch" : past_epoch+epoch+1,"model_state_dict" : my_unet.state_dict(),"opti_state_dict" : optimizer.state_dict(),"saved_losses" : df,"idx_test" : idx_test}, "resultsUnet/trial"+str(n_trial)+"/my_unet.pt")
...
#plt.xlabel("step")
#plt.ylabel("loss")
#plt.savefig("results/trial"+str(n_trial)+"/Training_L1.png")
#plt.close()
...
#plt.xlabel("step")
#plt.ylabel("loss")
#plt.savefig("results/trial"+str(n_trial)+"/Training_perc.png")
#plt.close()
...
#plt.xlabel("step")
#plt.ylabel("loss")
#plt.savefig("results/trial"+str(n_trial)+"/Test_L1.png")
#plt.close()
...
#plt.xlabel("step")
#plt.ylabel("loss")
#plt.savefig("results/trial"+str(n_trial)+"/Test_perc.png")
#plt.close()
...
plt.xlabel("step")
plt.ylabel("loss")
plt.savefig("resultsUnet/trial"+str(n_trial)+"/toutou.png")
plt.close()
...
plt.xlabel("step")
plt.ylabel("loss")
plt.savefig("resultsUnet/trial"+str(n_trial)+"/toutou2.png")
plt.close()

toutous = pd.DataFrame(data={"toutou : " : toutou, "toutou2 : " : toutou2})
toutous.to_csv("resultsUnet/trial"+str(n_trial)+"/toutous.csv",index=False)
...
plt.xlabel("step")
plt.ylabel("loss")
plt.savefig("resultsUnet/trial"+str(n_trial)+"/moutou.png")
plt.close()
...
plt.xlabel("step")
plt.ylabel("loss")
plt.savefig("resultsUnet/trial"+str(n_trial)+"/moutou2.png")
plt.close()
...
plt.xlabel("step")
plt.ylabel("-log(lr)")
plt.savefig("resultsUnet/trial"+str(n_trial)+"/lrs.png")
plt.close()
