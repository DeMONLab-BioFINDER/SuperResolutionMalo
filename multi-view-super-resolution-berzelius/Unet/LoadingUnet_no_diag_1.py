...
n_trial = 1 #create a folder "resultsUnet" with folders "triali" where i=n_trial to save loss curves, "params.txt"  saves the settings
...
info = pd.read_csv(big_path+"Unet/contents_ano_1/patient_info_anon_1"+".csv")
...
# Create bot_abs expected by the original code
if "bot_abs" not in info.columns:
    info["bot_abs"] = np.maximum(info["bot_1"], info["bot_first_slice"])
...
contexts = np.load(big_path+"Unet/contents_ano_1/contexts1_s"+str(n_colors)+".npy")
...
for i in range(136):
    if i in idx_test:
        I3T = nib.load(big_path+"Unet/contents_ano_1/images/hd_af_"+str(i)+"_1_s"+str(n_colors)+".nii.gz")
...
x = np.load(big_path+"Unet/contents_ano_1/inputs1_s"+str(n_colors)+".npy")
...
y = np.load(big_path+"Unet/contents_ano_1/outputs1_s"+str(n_colors)+".npy")
y[y>1.5] = 1.5
