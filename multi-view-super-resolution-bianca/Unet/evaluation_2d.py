...
def safe_ssim_2d(im_true_2d: np.ndarray, im_gen_2d: np.ndarray, data_range: float = 1.0):
    """
    Compute 2D SSIM safely for varying slice sizes.
    """
    h, w = im_true_2d.shape
    win = min(7, h, w)
    if win % 2 == 0:
        win -= 1
    if win < 3:
    ...
    return structural_similarity(im_true_2d, im_gen_2d, data_range=data_range, win_size=win)


def compute_metrics_slicewise(im_gen_01: np.ndarray, im_true: np.ndarray, slice_axis: int):
    """
    Full-slice 2D PSNR/SSIM, averaged over all slices along slice_axis.
    """
    n_slices = im_true.shape[slice_axis]

    psnrs = []
    ssims = []

    for s in range(n_slices):
        gt2d = get_slice(im_true, slice_axis, s)
        gen2d = get_slice(im_gen_01, slice_axis, s)

        psnr = peak_signal_noise_ratio(gt2d, gen2d, data_range=1)
        ssim = safe_ssim_2d(gt2d, gen2d, data_range=1)

        psnrs.append(float(psnr))
        ssims.append(float(ssim))

    return float(np.nanmean(psnrs)), float(np.nanmean(ssims))


def compute_metrics_slicewise_masked(im_gen_01: np.ndarray, im_true: np.ndarray, mask: np.ndarray, slice_axis: int):
    """
    Slice-wise masked 2D PSNR/SSIM averaged across valid slices.

    For each slice:
      - keep only masked pixels
      - PSNR computed on masked pixels
      - SSIM computed on 1D masked vectors for consistency with your existing masked logic
    """
    n_slices = im_true.shape[slice_axis]
    ...
    for s in range(n_slices):
        gt2d = get_slice(im_true, slice_axis, s)
        gen2d = get_slice(im_gen_01, slice_axis, s)
        m2d = get_slice(mask, slice_axis, s).astype(bool)
        ...
        psnr = peak_signal_noise_ratio(gt_vec, gen_vec, data_range=1)

        # SSIM on masked 1D vector; need enough samples for win_size
        if gt_vec.size < 7:
            ssim = np.nan
        else:
            win = min(7, gt_vec.size)
            if win % 2 == 0:
                win -= 1
            if win < 3:
                ssim = np.nan
            else:
                ssim = structural_similarity(gt_vec, gen_vec, data_range=1, win_size=win)
...
def main():
    ap = argparse.ArgumentParser("GAN 2D evaluation from 3D nifti (metrics csv)")
    ...
    # Corrupt slice handling / valid range definition
    ap.add_argument("--eval-axis", type=int, default=1, choices=[0, 1, 2],
                    help="Axis used for valid_ranges_x / bot_x / top_x when building no_corrupt mask.")
    ...
    # Perceptual loss (still 3D, kept same as original)
    ...
    # Logging
    ap.add_argument("--print-ratio", action="store_true", help="Print len(masked_gen)/len(all) ratio per subject")
    ...
    df = {
        "psnr_2d": [], "ssim_2d": [],
        "psnr_brain_2d": [], "ssim_brain_2d": [],
        "psnr_no_corrupt_2d": [], "ssim_no_corrupt_2d": [],
        "perc_10": [], "perc_50": [],
        "patient": []
    }
    ...
    for i in range(args.start_idx, args.end_idx + 1):
        ...
        # perceptual loss remains volume-wise, same as original
        if args.enable_perceptual:
            with torch.no_grad():
        ...
        # full slice-wise 2D metrics
        psnr_2d, ssim_2d = compute_metrics_slicewise(im_gen_01, im_true, args.slice_axis)
        df["psnr_2d"].append(float(psnr_2d))
        df["ssim_2d"].append(float(ssim_2d))
        ...
        if args.print_ratio:
            ratio = float(np.sum(mask_brain.flatten()) / mask_brain.size) if mask_brain.size > 0 else np.nan
            print(f"patient {i}: brain ratio = {ratio:.6f}")

        psnr_b2d, ssim_b2d = compute_metrics_slicewise_masked(im_gen_01, im_true, mask_brain, args.slice_axis)
        df["psnr_brain_2d"].append(float(psnr_b2d))
        df["ssim_brain_2d"].append(float(ssim_b2d))

        # no_corrupt metrics
        if args.disable_no_corrupt:
            df["psnr_no_corrupt_2d"].append(np.nan)
            df["ssim_no_corrupt_2d"].append(np.nan)
        else:
            ...
            psnr_nc2d, ssim_nc2d = compute_metrics_slicewise_masked(im_gen_01, im_true, mask_s, args.slice_axis)
            df["psnr_no_corrupt_2d"].append(float(psnr_nc2d))
            df["ssim_no_corrupt_2d"].append(float(ssim_nc2d))
    ...
    df["psnr_2d"].append(safe_mean(df["psnr_2d"]))
    df["ssim_2d"].append(safe_mean(df["ssim_2d"]))
    df["psnr_brain_2d"].append(safe_mean(df["psnr_brain_2d"]))
    df["ssim_brain_2d"].append(safe_mean(df["ssim_brain_2d"]))
    df["psnr_no_corrupt_2d"].append(safe_mean(df["psnr_no_corrupt_2d"]))
    df["ssim_no_corrupt_2d"].append(safe_mean(df["ssim_no_corrupt_2d"]))
